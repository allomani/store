...
if ($action == "edit") {
    ...
    if (is_array($stng)) {
        foreach ($stng as $key => $val) {

            db_query("update store_settings set value='" . db_escape($val, false) . "' where name='" . db_escape($key) . "'");
        }
    }
}
...
for ($i = 0; $i < count($orderby_checks); $i++) {
    ...
    if ($keyvalue == $settings['products_default_orderby']) {
        $chk = "selected";
    } else {
        $chk = "";
    }
...
if ($settings['products_default_sort'] == "asc") {
    $chk1 = "selected";
    $chk2 = "";
} else {
    $chk1 = "";
