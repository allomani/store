...
//if($datastring){$start = intval($start)+intval($perpage);unset($datastring);  }
...
if($send_to=="all"){
       $qr = db_query("select id,username,email from store_clients where usr_group > 0 order by id limit $start,$perpage");
        }else{
        $qr = db_query("select id,username,email from store_clients where username='".db_escape($username)."'");
        }


         if(db_num($qr)){

             $count_dt = db_qr_fetch("select count(id) as count from store_clients where  usr_group > 0 order by id");
        ...
        db_query("insert into store_clients_msgs (user,sender,title,content,date) values('$data[id]','".db_escape($from_name)."','".db_escape($from_subject)."','".db_escape($from_msg)."',now())");
      ...
      print "<li> $phrases[cp_mailing_sending_to] $data[username] .. " ;
         data_flush();
      $mailResult =  send_email($from_name,$from_email,$data['email'],$from_subject,$from_msg,$from_use_html,$from_encoding);
