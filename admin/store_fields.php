...
//---------------------- Store Fields ---------------------
if($action=="store_fields" || $action=="store_fields_edit_ok" || $action=="store_fields_add_ok" || 
$action=="store_fields_del" || $action=="store_fields_disable" || $action=="store_fields_enable"){

if_admin("store_fields");

//------- enable / disale -----------//
if($action=="store_fields_disable"){
        db_query("update store_fields_sets set active=0 where id='$id'");
        }

if($action=="store_fields_enable"){

       db_query("update store_fields_sets set active=1 where id='$id'");
        }
//-------- del -------//
if($action=="store_fields_del"){
$id=intval($id);
...
//----- edit -----//
if($action=="store_fields_edit_ok"){
$id=intval($id);
if($name){
...
<p align=$global_align><a href='index.php?action=store_fields_add'><img src='images/add.gif' border=0> $phrases[store_field_add] </a></p>
...
<td width=400><b>$phrases[the_name]</b></td><td width=400><b>$phrases[the_title]</b></td>

<td width=35><b>$phrases[fields_search_menu]</b></td>
<td width=35><b>$phrases[short_description]</b></td>
<td width=35><b>$phrases[product_details]</b></td>


<td width=300><b>$phrases[the_options]</b>
</td></tr></table>
</td></tr>
<tr><td width=100%>
...
<td width=300>";
          ...
          if($data['active']){
                        print "<a href='index.php?action=store_fields_disable&id=$data[id]'>$phrases[disable]</a> - " ;
                        }else{
                        print "<a href='index.php?action=store_fields_enable&id=$data[id]'>$phrases[enable]</a> - " ;
                        }
                        
print "<a href='index.php?action=store_fields_edit&id=$data[id]'>$phrases[edit]</a> - <a href='index.php?action=store_fields_del&id=$data[id]' onClick=\"return confirm('$phrases[are_you_sure]');\">$phrases[delete]</a></td></tr>
</table></div>";
}
...
print "<script type=\"text/javascript\">
        init_sortlist('store_fields_data_list','set_store_fields_sort');
</script>";
}else{
...
//---------- Add Store Field -------------
if($action=="store_fields_add"){
 if_admin("store_fields");

 print "<img src='images/link.gif'> <a href='index.php?action=store_fields'>$phrases[products_fields]</a> / $phrases[store_field_add]<br><br>";

 
print "<center>
<p align=center class=title>$phrases[store_field_add]</p>
<form action=index.php method=post name=sender>
<input type=hidden name=action value='store_fields_add_ok'>
...
//---------- Edit Store Field -------------
if($action=="store_fields_edit" || $action=="fields_options_edit_ok"){

    if_admin("store_fields");
$id=intval($id);

print "<img src='images/link.gif'> <a href='index.php?action=store_fields'>$phrases[products_fields]</a><br><br>";
...
if(db_num($qr)){
$data = db_fetch($qr);
print "<center><form action=index.php method=post name=sender>
<input type=hidden name=action value='store_fields_edit_ok'>
<input type=hidden name=id value='$id'>
<table width=80% class=grid>";
print "
...
print "<br><br>
<a href='index.php?action=fields_options_edit&id=$data[id]'>$phrases[options_edit]</a></td></tr>
...
print "<img src='images/link.gif'> <a href='index.php?action=store_fields'>$phrases[products_fields]</a> / <a href='index.php?action=store_fields_edit&id=$id'>$data_field[name]</a><br><br>";
...
if($action=="fields_options_del"){
    ...
    <input type=hidden name=id value=\"$id\">  
    <table width=80% class=grid>
    <tr><td><b>$phrases[the_value] : </b></td><td><input type=text name=\"value\" size=30></td>  <td rowspan=2><input type=submit value=\"$phrases[add_button]\"></td>
    </tr>
    <tr>
                <td><b>$phrases[the_image]</b></td>
                <td>
    ...
    if(db_num($qr)){
        ...
        <input type=hidden name=id value=\"$id\">
        <table width=80% class=grid><tr><td width=100%>
...
<table width=100%>
      ...
      <td><b>".($i+1)."</b></td>
            ...
            <td><a href='index.php?action=fields_options_del&id=$id&option_id=$data[id]'>$phrases[delete]</a></td>
            </tr></table></div>";
            $i++;
        }
        print "
        ...
        init_sortlist('store_fields_options_data_list','set_store_fields_options_sort');
</script>
        ";
    }else{
