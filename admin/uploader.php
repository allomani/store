...
if($uploader_thumb_hieght <=0){$uploader_thumb_hieght=100;}
    ...
    if($thumb_saved){
      ...
      if($replace_exists){
     @rename($thumb_saved,$saveto_filename);
     $thumb_saved = $saveto_filename;
     }
...
$phrases[auto_photos_resize]  ($phrases[cp_photo_resize_width] : <input type=text name=uploader_thumb_width size=2 value=\"$settings[uploader_thumb_width]\"> &nbsp;&nbsp;$phrases[cp_photo_resize_height]: <input type=text name=uploader_thumb_height size=2 value=\"$settings[uploader_thumb_hieght]\"> <input type=\"checkbox\" name=\"fixed\" value=1>$phrases[fixed])
</fieldset>";
